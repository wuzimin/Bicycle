...
namespace Bicycle
{
    ...
    public sealed partial class collection : Page
    {
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            try
            { //展示某个用户的收藏
                using (var statement = db.Prepare("SELECT title, age, price, phonenumber, school, description, imagesource FROM favorite WHERE username = ?"))
                {
        ...
        private void delete_Click(object sender, RoutedEventArgs e) //取消收藏，从favorite中删除
        {
            if (title.Text == "")
            {
            ...
            else
            {
                ...
                try
                {
                    using (var statement = db.Prepare("DELETE FROM favorite WHERE title = ?"))
                    {
                        statement.Bind(1, title.Text);
                        statement.Step();
                        Frame.Navigate(typeof(collection), username);
                    }
                }
                catch (Exception ex)
                {
                ...
                var info = new MessageDialog("取消成功！").ShowAsync();
            }
        }
